...
| 2단계 UI — U2 해상도 QA | ✅ 완료 (MainMenu·StudyScene 4개 화면비 검수, 이상 없음) |
| 2단계 UI — U3 한글 가독성 | ✅ 완료 (힌트 글자 대비 개선, 나머지 정상) |
| 2단계 UI — U4 폴리시 | ✅ 완료 (씬 전환·버튼 피드백·결과 팝업) |
| 2단계 UI — U5 아이콘/스토어 | ⏸ **남은 과제** — 자산 완비, 새 아트워크는 디자인 입력 필요 (코드 실행 불가) |

> **오늘(2026-07-24) 리뉴얼 세션 종료 시점**: 코드로 실행 가능한 항목은 1단계(안정성)·2단계 U2~U4 모두 완료·검증·커밋. 남은 것은 **U5 아이콘/스토어 아트워크(디자인 몫)** 와 **iOS 실기기 검증(D1·D2)**, 그리고 아래 백로그.

---

## App Store 리젝 2.3.8 대응 — Kids 카테고리(방법 B)

2.0.4 심사에서 **Guideline 2.3.8**: 앱 이름 "우리아이 한글떼기 LITE"가 아동 전용을 암시하나 Kids 카테고리 미제출. → **방법 B(Kids 카테고리 등록)** 선택으로 **브랜드·이름·로고 유지**하고, Kids 규정인 **부모 인증 게이트**를 추가.

| 항목 | 상태 |
|------|------|
| 부모 게이트 컴포넌트(`ParentalGate`) | ✅ 구현 완료 (곱셈 키패드, Windows 빌드·렌더 검증, 커밋 `4d047fb`) |
| 배선: 결제(cfStep2~5·cfTotal)·공유(cfShare)·정식버전(cfFullVersion) | ✅ 완료 |
| **iOS Xcode 타깃에 `ParentalGate.cpp/.h` 추가** | ⏸ **Mac에서 직접** (iOS는 커밋된 xcodeproj 사용, BUILD.md 표 반영). 안 하면 링크 실패 |
| **App Store Connect: "Made for Kids" 선택 + 연령대 설정** | ⏸ **웹 콘솔에서** (무료·유료 앱 모두) |
| 재심사 회신/재제출 | ⏸ 위 완료 후 |

> Kids 나머지 요건(광고 없음·개인정보 수집 없음·오프라인)은 이미 충족. privacy.html의 "어린이용 설계" 문구도 방법 B와 오히려 일치.
...
(버튼에 구워진 `홈`·`모든 학습`·`쉽게하기`는 이미 한글)

**⚠️ 작업 시 두 함정 필수 처리** (이미 이번 세션 초기화 팝업에서 확인됨):
1. `PrintStyle` 기본 폰트가 `arial.ttf`(**한글 글리프 없음**) → 한글은 반드시 **`KR_FONT_TTF`(malgun)** 를 넘겨야 렌더됨.
2. 이 파일은 UTF-8(pragma 없음)이라 `UTF8()` 매크로가 **Win32 에서 이중변환으로 한글을 깨뜨림** → **UTF-8 바이트 직접 지정**(예: `"\xEA\xB2\x8C..."`) 사용. (초기화 팝업 커밋 `49168fe` 참고)
3. 각 라벨이 코드 렌더(PrintStyle/Label)인지 이미지(버튼 굽기)인지 구분해 처리.
> 참고: 이는 U1(현지화, 제외 확정)이 아니라 **주 타겟(한국어) 텍스트 정정**임. [[project_no_localization]] 와 혼동 말 것.

### B-UI-2. 시작화면(MainMenu) 특정 영역 시각 구성 개선

시작화면의 한 영역을 더 예쁘게 구성 요청.
**⚠️ 대상 영역 미확정** — 요청 시 첨부 이미지가 크기 초과로 확인 불가했음. 다음 작업 시 **어느 부분인지 재확인 필요**.
...
| U2 | ✅ **해상도 QA 완료 — 이상 없음.** Win32 기종 프리셋 시뮬(`kw4_x.exe <기종>`)로 MainMenu·StudyScene을 4개 화면비(1.33 iPad Pro13·1.52 iPad mini6·1.78 iPhone8·2.17 iPhone17Pro)에서 캡처 검수. 잘림·여백·요소충돌 없음, `-hd`/`-hdx` 전환·ACTIVE_HEIGHT 로직 정상 | `FIXED_WIDTH` + `-hd`/`-hdx` | (참고) 실기기 최종 확인은 여전히 권장. 신규 UI 변경 시 이 프리셋으로 회귀 검수 |
| U3 | ✅ **가독성 검수·개선 완료.** 글자 버튼·메뉴·레벨 라벨은 크고 대비 좋음(변경 불필요). malgun 렌더 정상. **힌트 글자만 회색(120)→밝은 톤(240)+투명도(180)로 개선** — 어두운 정답칸 위 유아 가독성 확보 | StudyScene `ShowHint`, `PrintStyle` | Windows 빌드 캡처로 before/after 검증 |
| U4 | ✅ **폴리시 완료** (우선순위: 전환>피드백>팝업) | MainMenu/Study/AppleTree/Info | ① **씬 전환**: 이전 버튼 전환 버려지던 버그 수정 + 시간/방향 통일(SCENE_TRANSITION_TIME, 앞=SlideInL·뒤=SlideInR) + 정보 전환 추가 ② **버튼 피드백**: 무음 버튼(정보·칭찬나무·이전)에 클릭음 ③ **결과 팝업**: 초기화 안내 영어(오타)→한글(바이트 리터럴+malgun) |
| U5 | ⏸ **남은 과제 (디자인 입력 필요)** | 아이콘 세트·스토어 이미지 모두 완비(누락 없음) | 자산은 있으나 **새 아트워크 제작은 그래픽 디자인 작업**이라 코드로 실행 불가. 필요 시: (a) 리뉴얼 앱 스토어 스크린샷 재생성(캡처 하네스 활용), (b) 새 1024px 원본 제공 시 전 크기 리사이즈 스크립트 작성, (c) [store_listing.md](store_listing.md) 문구 검토 |
