...
| # | 항목 | 근거/현황 | 방향 |
|---|------|-----------|------|
| U1 | **현지화 부재** — 버튼/문구가 한글 이미지에 구워져 있고 i18n 없음. 비한국어권 사용자 ~10% | 코드에 cocos 현지화 흔적 없음, `UI4HD/*Btn*` 이미지에 텍스트 포함 | 최소 영어 대응 검토(문자열/이미지 분리 또는 영어 에셋 추가). 범위·비용 큰 편이라 별도 판단 |
...
5. → 1단계 안정화 후 **2단계 UI** 착수. U2(해상도 QA)로 현재 상태 기준선 잡고 U3~U4 순차 개선. U1(현지화)은 범위가 커서 별도 의사결정.
